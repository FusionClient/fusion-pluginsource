...
public interface OneClickThievingConfig extends Config {
	...
	@ConfigItem(
		keyName = "enablePray",
		name = "Use Prayer",
		description = "This will use redemption with prayer/restore potions<br>This is mainly for pickpocketing elves/vyres",
		position = 5
	)
	...
	@ConfigItem(
		keyName = "prayMethod",
		name = "Pray Method",
		description = "Lazy reactivates redemption when possible. Active only enables when your hp is low<br>Active should only be used if your max hp is greater than 60",
		position = 6,
		hidden = true,
		unhide = "enablePray"
	)
