    ...
    protected void startUp()
    {
        ...
        titleBarButton = NavigationButton.builder()
                ...
                .onClick(() ->
                {
                    ...
                    CustomClientResizingProfile active = customclientresizingProfiles.stream()
                            .filter(CustomClientResizingProfile::isVisible).findFirst().orElse(null);
                    if (active == null)
                    {
                        return;
                    }
                    Rectangle bounds = new Rectangle(
                            active.getPosition().width, active.getPosition().height, active.getSize().width, active.getSize().height
                    );
                    if (!clientUi.isSidebarOpen())
                    {
                        bounds.width -= TOOLBAR_WIDTH;
                    }
                    if (clientUi.getPluginPanel() == null)
                    {
                        bounds.width -= (PluginPanel.PANEL_WIDTH + SCROLLBAR_WIDTH);
                    }
