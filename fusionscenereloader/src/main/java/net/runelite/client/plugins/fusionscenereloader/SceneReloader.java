package net.runelite.client.plugins.fusionscenereloader;
...
public class SceneReloader extends Plugin implements KeyListener {
    ...
    @Inject
    private SceneReloaderConfig config;

    @Inject
    private ClientToolbar clientToolbar;

    private SceneReloaderPanel panel;
    ...
    @Provides
    SceneReloaderConfig getConfig(ConfigManager configManager) {
        return configManager.getConfig(SceneReloaderConfig.class);
    }
    ...
    public void CreatePanel() {
        ...
        panel = injector.getInstance(SceneReloaderPanel.class);
        panel.init();
    ...
    public void onGameTick(GameTick event)
    {
        ...
        if (config.showPanel() != SceneReloaderConfig.panelMode.OFF) {
            if (raidsOnly != buttonAttatched && config.showPanel() == SceneReloaderConfig.panelMode.RAIDS) {
                SwingUtilities.invokeLater(() -> {
                    if (raidsOnly) {
            ...
            } else if (config.showPanel() == SceneReloaderConfig.panelMode.LOGGED_IN) {
                if (!buttonAttatched) {
