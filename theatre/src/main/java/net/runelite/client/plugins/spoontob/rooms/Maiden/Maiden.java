...
public class Maiden extends Room {
    ...
    public void load() {
        ...
        overlayManager.add(maidenMaxHitTooltip);
    }
    ...
    public void unload() {
        ...
        overlayManager.remove(maidenMaxHitTooltip);
    ...
    public void onClientTick (ClientTick event){
        ...
        if(maidenNPC != null && maidenActive) {
            if (maidenCrabInfoList.size() > 0 && nyloSpawnDelay == 0) {
                for (MaidenCrabInfo mci : maidenCrabInfoList) {
                    if(mci.crab.getHealthRatio() >= 0){
                        mci.hpRatio = mci.crab.getHealthRatio();
                        mci.hpScale = mci.crab.getHealthScale();
                    }
                }

                if (config.maidenCrabHpPriority() && maidenCrabInfoList.size() > 1) {
                    MenuEntry[] entries = client.getMenuEntries();
                    MenuEntry[] newEntries = client.getMenuEntries();
                    int highestHp = 0;

                    for (int i = entries.length - 1; i >= 0; i--) {
                        String target = Text.removeTags(entries[i].getTarget()).toLowerCase();
                        String option = Text.removeTags(entries[i].getOption()).toLowerCase();

                        if(client.getCachedNPCs().length > entries[i].getIdentifier()) {
                            NPC npc = client.getCachedNPCs()[entries[i].getIdentifier()];
                            if (target.startsWith("nylocas matomenos") && (option.equals("attack") || (option.contains("cast ") && option.contains(" ->")))&& npc != null) {
                                for (MaidenCrabInfo mci : maidenCrabInfoList) {
                                    if (mci.crab.getIndex() == npc.getIndex()) {
                                        if (mci.crab.getHealthRatio() > highestHp) {
                                            highestHp = mci.crab.getHealthRatio();
                                            MenuEntry tempEntry = newEntries[newEntries.length - 1];
                                            newEntries[newEntries.length - 1] = entries[i];
                                            entries[i] = tempEntry;
                                        }
                                        break;
                                    }
